...
def payment():
    if request.method == "POST":
        ...
        total_amount = session.get("total_bill", 0)

        # Cash on Delivery case (agar cart me kuch hai to allow karo)
        ...
        # UPI Payment case
            ...
            if total_amount <= 0:
                return "Invalid amount. Please check your cart."

            # Frontend se entered amount lena
            entered_amount = request.form.get("amount", type=float)

            if entered_amount is None:
                return "Amount not provided."

            if entered_amount < total_amount:
                return f"Amount should be at least ₹{total_amount}"

            return render_template("payment.html", total_amount=total_amount)

    # GET request — by default UPI section hidden
    return render_template("payment.html", total_amount=session.get("total_bill", 0))
...
def download_bill():
    order_id = session.get("last_order_id")
    if not order_id:
        return "No recent order to download", 400
    ...
    html = render_template("bill_pdf.html", order=order, items=items)
